...
def get_loaders(
    ...
    if verbose:
        ...
        print(
            f"INFO: {'no-overlapping' if window_step else 'overlapping'} processed dataframe"
        )
    ...
    for i, driver in enumerate(drivers):
        ...
        df["speed_mean"] = (
            df["speed"]
            .rolling(window=window_size, min_periods=1, step=window_step)
            .mean()
        )
        df["speed_std"] = (
            df["speed"]
        ...
        df["RPM_std"] = (
            df["RPM"].rolling(window=window_size, min_periods=1, step=window_step).std()
        )
        df["brake_mean"] = (
            df["Brake pedal"]
            .rolling(window=window_size, min_periods=1, step=window_step)
    ...
    for i, driver in enumerate(drivers):
        ...
        for train_index, test_index in tscv.split(X_train):
            ...
            y_train_fold, y_val_fold = (
                y_train_driver_fold.iloc[train_index],
                y_train_driver_fold.iloc[test_index],
            )
        ...
        if verbose:
            ...
            print(
                classification_report(
                    y_test_driver_fold,
                    lstm_y_pred if model_type != "logreg" else model_test_pred,
